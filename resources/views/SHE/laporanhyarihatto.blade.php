        ...
        <div class="card-body p-4">
            ...
            <div class="row g-3 justify-content-between align-items-center mb-4">
                <div class="col-md-6 col-lg-4">
                    <form action="{{ url('/she/laporanhyarihatto') }}" method="GET">
                        <div class="input-group search-group">
                            <input type="text" id="searchInput" name="search" value="{{ request('search') }}" class="form-control border-end-0" placeholder="Cari data...">
                ...
                <div class="col-md-auto">
                    <a href="{{ url('/she/hyari-hatto/export/excel') }}" class="btn btn-success btn-modern shadow-sm text-white">
                            ...
                            <td>
                                @if($laporan->ptas->count() > 0)
                                    @foreach($laporan->ptas as $pta)
                                        <span class="badge bg-light text-dark border badge-pill mb-1">{{ $pta->nama_pta }}</span>
                                    @endforeach
                                @else
                            ...
                            <td>
                                @if($laporan->ktas->count() > 0)
                                    @foreach($laporan->ktas as $kta)
                                        <span class="badge bg-light text-dark border badge-pill mb-1">{{ $kta->nama_kta }}</span>
                                    @endforeach
                                @else
                                    <span class="text-muted small">N/A</span>
                                @endif
                            ...
                            <td>
                                @if($laporan->pbs->count() > 0)
                                    @foreach($laporan->pbs as $pb)
                                        <span class="badge bg-danger-subtle text-danger border border-danger badge-pill mb-1">{{ $pb->nama_pb }}</span>
                                    @endforeach
                                @else
                                    <span class="text-muted small">N/A</span>
                                @endif
                            ...
                            <td class="text-center">
                                <div class="btn-group">
                                    ...
                                    <a href="{{ url('/she/laporanhyarihatto/download/' . $laporan->id) }}" 
                                       class="btn btn-sm btn-outline-success rounded-circle me-1" title="Download Laporan">
    ...
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable"> 
        <div class="modal-content">
            ...
            <div class="modal-body p-4 bg-light">
                <form action="{{ url('/she/laporanhyarihatto/update/' . $laporan->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="col-lg-7">
                            <div class="card border-0 shadow-sm rounded-4 mb-3">
                                <div class="card-header bg-white border-bottom py-3">
                                    <h6 class="mb-0 fw-bold"><i class="bi bi-info-circle me-2 text-info"></i>Detail Temuan</h6>
                                </div>
                                <div class="card-body">
                                    <div class="row mb-4">
                                        <div class="col-md-6 mb-3">
                                            <label class="small text-muted text-uppercase fw-bold">Perilaku Tidak Aman</label>
                                            <div class="mt-2">
                                                @php $selectedPtas = $laporan->ptas->pluck('id')->toArray(); @endphp
                                                @foreach($masterPtas as $pta)
                                                    @if(in_array($pta->id, $selectedPtas))
                                                    <span class="badge bg-secondary-subtle text-dark border me-1 mb-1">{{ $pta->nama_pta }}</span>
                                                    @endif
                                                @endforeach
                                            </div>
                                        </div>
                                        <div class="col-md-6 mb-3">
                                            <label class="small text-muted text-uppercase fw-bold">Kondisi Tidak Aman</label>
                                            <div class="mt-2">
                                                @php $selectedKtas = $laporan->ktas->pluck('id')->toArray(); @endphp
                                                @foreach($masterKtas as $kta)
                                                    @if(in_array($kta->id, $selectedKtas))
                                                    <span class="badge bg-secondary-subtle text-dark border me-1 mb-1">{{ $kta->nama_kta }}</span>
                                                    @endif
                                                @endforeach
                                            </div>
                                        </div>
                                    ...
                                    <div class="mb-4">
                                        <label class="small text-muted text-uppercase fw-bold">Deskripsi Kejadian</label>
                                    ...
                                    <div class="mb-4">
                                        <label class="small text-muted text-uppercase fw-bold">Usulan Countermeasure</label>
                                        <div class="p-3 bg-light rounded-3 mt-2 border">
                                            {{ $laporan->usulan }}
                                        </div>
                                    </div>
                        ...
                        <div class="col-lg-5">
                            ...
                            <div class="card border-0 shadow-sm rounded-4 mb-4">
                                <div class="card-body p-2">
                                    @if($laporan->bukti)
                                        <img src="{{ asset('storage/' . $laporan->bukti) }}" class="img-fluid rounded-3 w-100 shadow-sm" alt="Bukti Visual">
                                    @else
                            ...
                            <div class="card border-warning shadow-sm rounded-4">
                                <div class="card-header bg-warning text-dark border-0 py-3">
                                    <h6 class="mb-0 fw-bold"><i class="bi bi-clipboard-check me-2"></i>Rekomendasi P2K3</h6>
                                </div>
                                <div class="card-body">
                                    {{-- Hidden Inputs --}}
                                    @foreach($selectedPtas as $pta_id) <input type="hidden" name="pta_id[]" value="{{ $pta_id }}"> @endforeach
                                    @foreach($selectedKtas as $kta_id) <input type="hidden" name="kta_id[]" value="{{ $kta_id }}"> @endforeach
                                    @php $selectedPbs = $laporan->pbs->pluck('id')->toArray(); @endphp
                                    @foreach($selectedPbs as $pb_id) <input type="hidden" name="pb_id[]" value="{{ $pb_id }}"> @endforeach
                                    <input type="hidden" name="deskripsi" value="{{ $laporan->deskripsi }}">
                                    <input type="hidden" name="usulan" value="{{ $laporan->usulan }}">

                                    <div class="mb-0">
                                        <label for="edit_rekomendasi_{{ $laporan->id }}" class="form-label fw-bold small">Masukan Rekomendasi <span class="text-danger">*</span></label>
