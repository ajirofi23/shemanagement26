...
<div class="content p-3">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <button type="button" class="btn btn-info shadow-sm text-white" data-bs-toggle="modal" data-bs-target="#addSafetyPatrolModal">
                            ...
                            <td>
                                <div class="d-flex flex-wrap gap-1">
                                    ...
                                    <button type="button" class="btn btn-sm btn-primary" 
                                        data-bs-toggle="modal" 
                                        data-bs-target="#editSafetyPatrolModal{{ $laporan->id }}"
                                        title="Edit Data">
                                    ...
                                    <form action="{{ url('/she/safety-patrol/destroy/' . $laporan->id) }}" method="POST" style="display:inline-block;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Yakin hapus Laporan Safety Patrol ini?')" title="Hapus Data">
...
<div class="modal fade" id="addSafetyPatrolModal" tabindex="-1" aria-labelledby="addSafetyPatrolModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg"> 
        <div class="modal-content">
            <div class="modal-header bg-info text-white">
                <h5 class="modal-title" id="addSafetyPatrolModalLabel"><i class="bi bi-plus-circle-fill me-1"></i> Buat Laporan Safety Patrol</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form action="{{ url('/she/safety-patrol/store') }}" method="POST" enctype="multipart/form-data" id="addSafetyPatrolForm">
                    @csrf
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="eporte" id="eporte" class="form-control" value="{{ old('eporte') }}" placeholder="Masukkan E-PORTE" required>
                        </div>

                        <div class="col-md-6 mb-3">
                            <label for="area" class="form-label">Area <span class="text-danger">*</span></label>
                            <input type="text" name="area" id="area" class="form-control" value="{{ old('area') }}" placeholder="Contoh: Workshop, Kantor, Gudang" required>
                        </div>
                        ...
                        <div class="col-12 mb-3">
                            ...
                            <textarea name="problem" id="problem" class="form-control" rows="3" placeholder="Deskripsikan masalah yang ditemukan" required>{{ old('problem') }}</textarea>
                        </div>

                        <div class="col-12 mb-3">
                            <label for="counter_measure" class="form-label">Counter Measure <span class="text-danger">*</span></label>
                            <textarea name="counter_measure" id="counter_measure" class="form-control" rows="3" placeholder="Tindakan yang diperlukan" required>{{ old('counter_measure') }}</textarea>
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="file" name="foto_before" id="foto_before" class="form-control" accept="image/*" onchange="previewImage(this, 'fotoBeforePreview')">
...
<div class="modal fade" id="tindakLanjutModal{{ $laporan->id }}" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ url('/she/safety-patrol/tindak-lanjut/' . $laporan->id) }}" 
                      method="POST" 
                    ...
                    <div class="alert alert-info">
                        ...
                        <ul class="mb-0 mt-2">
                            ...
                            <li>Due Date: <strong>{{ \Carbon\Carbon::parse($laporan->due_date)->format('d-m-Y') }}</strong></li>
                        </ul>
                    </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <ul class="nav nav-tabs mb-4" id="detailTab{{ $laporan->id }}" role="tablist">
                    ...
                    @if($laporan->foto_after)
                    <li class="nav-item" role="presentation">
                        <button class="nav-link {{ !$laporan->foto_before ? 'active' : '' }}" id="after-tab-{{ $laporan->id }}" data-bs-toggle="tab" data-bs-target="#after-{{ $laporan->id }}" type="button" role="tab">
                ...
                <div class="tab-content" id="detailTabContent{{ $laporan->id }}">
                    <!-- Tab Before -->
                    @if($laporan->foto_before)
                    <div class="tab-pane fade {{ $laporan->foto_before ? 'show active' : '' }}" id="before-{{ $laporan->id }}" role="tabpanel">
                        <div class="row">
                            <div class="col-md-6 offset-md-3 mb-3">
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="d-flex flex-wrap gap-1">
                                            <a href="{{ asset('storage/' . $laporan->foto_before) }}" target="_blank" class="btn btn-sm btn-primary">
                                                <i class="bi bi-eye"></i> Lihat
                                            </a>
                                            <a href="{{ asset('storage/' . $laporan->foto_before) }}" download class="btn btn-sm btn-success">
                                                <i class="bi bi-download"></i> Download
                                            </a>
                                            @if(isset($permission) && $permission->can_edit && $laporan->status === 'Open')
                                            <form action="{{ url('/she/safety-patrol/' . $laporan->id . '/delete-image/before') }}" 
                                                  method="POST" 
                                                  class="d-inline"
                    ...
                    <!-- Tab After -->
                    @if($laporan->foto_after)
                    <div class="tab-pane fade {{ !$laporan->foto_before ? 'show active' : '' }}" id="after-{{ $laporan->id }}" role="tabpanel">
                        <div class="row">
                            <div class="col-md-6 offset-md-3 mb-3">
                                <div class="card">
                                    <a href="{{ asset('storage/' . $laporan->foto_after) }}" target="_blank">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="d-flex flex-wrap gap-1">
                                            <a href="{{ asset('storage/' . $laporan->foto_after) }}" target="_blank" class="btn btn-sm btn-primary">
                                                <i class="bi bi-eye"></i> Lihat
                                            </a>
                                            <a href="{{ asset('storage/' . $laporan->foto_after) }}" download class="btn btn-sm btn-success">
                                                <i class="bi bi-download"></i> Download
                                            </a>
                                            @if(isset($permission) && $permission->can_edit)
                                            <form action="{{ url('/she/safety-patrol/' . $laporan->id . '/delete-image/after') }}" 
                                                  method="POST" 
                                                  class="d-inline"
    ...
    <div class="modal-dialog modal-lg"> 
        <div class="modal-content">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title"><i class="bi bi-pencil-square me-1"></i> Edit Laporan Safety Patrol #{{ $laporan->id }}</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <form action="{{ url('/she/safety-patrol/update/' . $laporan->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="edit_tanggal_{{ $laporan->id }}" class="form-label">Tanggal <span class="text-danger">*</span></label>
                            <input type="date" name="tanggal" id="edit_tanggal_{{ $laporan->id }}" class="form-control" value="{{ old('tanggal', \Carbon\Carbon::parse($laporan->tanggal)->format('Y-m-d')) }}" required>
                        </div>

                        <div class="col-md-6 mb-3">
                            <label for="edit_eporte_{{ $laporan->id }}" class="form-label">E-PORTE <span class="text-danger">*</span></label>
                            <input type="text" name="eporte" id="edit_eporte_{{ $laporan->id }}" class="form-control" value="{{ old('eporte', $laporan->eporte) }}" required>
                        </div>

                        <div class="col-md-6 mb-3">
                            <label for="edit_area_{{ $laporan->id }}" class="form-label">Area <span class="text-danger">*</span></label>
                            <input type="text" name="area" id="edit_area_{{ $laporan->id }}" class="form-control" value="{{ old('area', $laporan->area) }}" required>
                        </div>
                        ...
                        <div class="col-12 mb-3">
                            <label for="edit_problem_{{ $laporan->id }}" class="form-label">Problem <span class="text-danger">*</span></label>
                            <textarea name="problem" id="edit_problem_{{ $laporan->id }}" class="form-control" rows="3" required>{{ old('problem', $laporan->problem) }}</textarea>
                        </div>

                        <div class="col-12 mb-3">
                            <label for="edit_counter_measure_{{ $laporan->id }}" class="form-label">Counter Measure <span class="text-danger">*</span></label>
                            <textarea name="counter_measure" id="edit_counter_measure_{{ $laporan->id }}" class="form-control" rows="3" required>{{ old('counter_measure', $laporan->counter_measure) }}</textarea>
                        </div>

                        <div class="col-md-6 mb-3">
                            <label for="edit_due_date_{{ $laporan->id }}" class="form-label">Due Date <span class="text-danger">*</span></label>
                            <input type="date" name="due_date" id="edit_due_date_{{ $laporan->id }}" class="form-control" value="{{ old('due_date', \Carbon\Carbon::parse($laporan->due_date)->format('Y-m-d')) }}" required>
                        </div>
                    </div>
                        ...
                        @if($laporan->foto_before)
                            <div class="row">
                                <div class="col-md-4 mb-2">
                                    <div class="card">
                                        <a href="{{ asset('storage/' . $laporan->foto_before) }}" target="_blank">
                                            <img src="{{ asset('storage/' . $laporan->foto_before) }}" class="card-img-top" style="height: 150px; object-fit: cover;">
                                        </a>
                                        <div class="card-body p-2 text-center">
                                            ...
                                            <div class="form-check mt-2">
                                                <input class="form-check-input" type="checkbox" 
                                                       name="hapus_foto_before" 
                                                       id="hapus_foto_before_{{ $laporan->id }}" 
                                                       value="1">
                                                <label class="form-check-label text-danger" for="hapus_foto_before_{{ $laporan->id }}">
