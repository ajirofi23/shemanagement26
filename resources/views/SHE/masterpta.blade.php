...
@section('content')
    ...
    <div class="content p-4"> {{-- Kembali ke p-4 untuk spacing yang lebih lega/lux --}}
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
            ...
            <button type="button" class="btn btn-primary shadow-sm pulse-animation" data-bs-toggle="modal"
                data-bs-target="#addPtaModal">
                        ...
                        <form action="{{ url('/she/master/pta') }}" method="GET" class="d-flex">
                            <div class="input-group">
                                ...
                                <input type="text" id="searchInput" name="search" value="{{ request('search') }}"
                                    class="form-control border-start-0" placeholder="Cari data pta...">
                                        ...
                                        <div class="btn-group" role="group">
                                            @if(isset($permission) && $permission->can_edit)
                                                <button type="button" class="btn btn-sm btn-light text-primary border me-2"
                                                    data-bs-toggle="modal" data-bs-target="#editPtaModal{{ $pta->id }}"
                                                    title="Edit">
                                            ...
                                            @if(isset($permission) && $permission->can_delete)
                                                <form action="{{ url('/she/master/pta/destroy/' . $pta->id) }}" method="POST"
                                                    class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
...
@section('modals')
    ...
    <style>
        ...
        .form-control:focus {
            box-shadow: 0 0 0 0.25 luxury rgba(13, 110, 253, 0.1);
        }
    </style>
                ...
                <form action="{{ url('/she/master/pta/store') }}" method="POST">
                    ...
                    <div class="modal-body px-4">
                        <div class="mb-3">
                            ...
                            <textarea name="nama_pta" class="form-control bg-light border-0" rows="4"
                                placeholder="Masukkan deskripsi perilaku tidak aman..." required></textarea>
                        </div>
                    </div>
                        ...
                        <form action="{{ url('/she/master/pta/update/' . $pta->id) }}" method="POST">
                            ...
                            <div class="modal-body px-4">
                                <div class="mb-3">
                                    ...
                                    <textarea name="nama_pta" class="form-control bg-light border-0" rows="4"
                                        required>{{ $pta->nama_pta }}</textarea>
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-light px-4" data-bs-dismiss="modal">Batal</button>
