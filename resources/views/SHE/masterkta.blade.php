...
@section('content')
    ...
    <div class="content p-4 fade-in-up">
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
            ...
            <button type="button" class="btn btn-primary shadow-sm px-4 py-2" data-bs-toggle="modal"
                data-bs-target="#addKtaModal">
            ...
            <div class="card-body p-4">
                ...
                <div class="row g-3 mb-4">
                    ...
                    <!-- <div class="col-12 col-md-4 text-md-end">
                        <a href="{{ url('/she/master/kta/export') }}"
                            class="btn btn-outline-success w-100 w-md-auto shadow-sm">
                                        ...
                                        <div class="d-flex justify-content-center gap-2">
                                            @if(isset($permission) && $permission->can_edit)
                                                <button type="button" class="btn btn-sm btn-outline-primary shadow-sm"
                                                    data-bs-toggle="modal" data-bs-target="#editKtaModal{{ $kta->id }}"
                                                    title="Edit Data">
                                            ...
                                            @if(isset($permission) && $permission->can_delete)
                                                <form action="{{ url('/she/master/kta/destroy/' . $kta->id) }}" method="POST"
                                                    class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-sm btn-outline-danger shadow-sm"
                                                        onclick="return confirm('Yakin hapus data: {{ $kta->nama_kta }}?')"
                                                        title="Hapus Data">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content shadow-lg">
                <div class="modal-header">
                    <h5 class="modal-title fw-bold text-dark"><i class="bi bi-plus-circle-fill text-primary me-2"></i>
                        Tambah KTA</h5>
                ...
                <div class="modal-body">
                    <form action="{{ url('/she/master/kta/store') }}" method="POST">
                        @csrf
                        <div class="mb-3">
                            <label for="nama_kta" class="form-label fw-bold small">Deskripsi Kondisi Tidak Aman <span
                                    class="text-danger">*</span></label>
                            <textarea name="nama_kta" id="nama_kta" class="form-control border-light-subtle bg-light"
                                rows="4" placeholder="Jelaskan kondisi tidak aman..." required></textarea>
                            <div class="form-text mt-2"><i class="bi bi-info-circle me-1"></i> Contoh: Kabel terkelupas di
                                area produksi.</div>
                        </div>
        ...
        @if(isset($permission) && $permission->can_edit)
            <div class="modal fade" id="editKtaModal{{ $kta->id }}" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content shadow-lg">
                        <div class="modal-header">
                            <h5 class="modal-title fw-bold text-dark"><i class="bi bi-pencil-square text-primary me-2"></i> Edit
                                Data</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <form action="{{ url('/she/master/kta/update/' . $kta->id) }}" method="POST">
                                ...
                                @method('PUT')
                                <div class="mb-3">
                                    <label for="edit_nama_kta{{ $kta->id }}" class="form-label fw-bold small">Deskripsi Kondisi
                                        Tidak Aman <span class="text-danger">*</span></label>
                                    <textarea name="nama_kta" id="edit_nama_kta{{ $kta->id }}"
                                        class="form-control border-light-subtle bg-light" rows="4"
                                        required>{{ $kta->nama_kta }}</textarea>
                                </div>
