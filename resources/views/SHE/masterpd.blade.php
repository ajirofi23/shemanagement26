                                        ...
                                        <div class="d-flex justify-content-center gap-1">
                                            @if(isset($permission) && $permission->can_edit)
                                                <button type="button" class="btn btn-primary btn-action" data-bs-toggle="modal"
                                                    data-bs-target="#editPdModal{{ $pd->id }}" title="Edit Data">
                                                    <i class="bi bi-pencil-square"></i>
                                                </button>
                                            @endif

                                            @if(isset($permission) && $permission->can_delete)
                                                <form action="{{ url('/she/master/pd/destroy/' . $pd->id) }}" method="POST"
                                                    class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-danger btn-action"
                                                        onclick="return confirm('Yakin hapus data: {{ $pd->nama_pd }}?')"
                                                        title="Hapus Data">
...
@section('modals')
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content shadow-2xl">
                <div class="modal-header">
                    <h5 class="modal-title fw-bold text-dark" id="addPdModalLabel"><i
                            class="bi bi-file-earmark-plus-fill text-primary me-2"></i>Tambah Pelanggaran Dokumen</h5>
                ...
                <form action="{{ url('/she/master/pd/store') }}" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="nama_pd" class="form-label fw-bold">Deskripsi Pelanggaran <span
                                    class="text-danger">*</span></label>
                            <textarea name="nama_pd" id="nama_pd" class="form-control form-control-modern" rows="4" required
                                placeholder="Sebutkan jenis pelanggaran..."></textarea>
                            <div class="form-text mt-2"><i class="bi bi-info-circle me-1"></i> Contoh: Tanggal tidak diisi,
                                Nama tidak jelas.</div>
                        </div>
                    </div>
    ...
    @foreach($pds as $pd)
        @if(isset($permission) && $permission->can_edit)
            <div class="modal fade" id="editPdModal{{ $pd->id }}" tabindex="-1" aria-labelledby="editPdModalLabel{{ $pd->id }}"
                aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content shadow-2xl">
                        <div class="modal-header">
                            <h5 class="modal-title fw-bold text-dark" id="editPdModalLabel{{ $pd->id }}"><i
                                    class="bi bi-pencil-square text-primary me-2"></i>Edit Data</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <form action="{{ url('/she/master/pd/update/' . $pd->id) }}" method="POST">
                            ...
                            <div class="modal-body">
                                <div class="mb-3">
                                    <label for="edit_nama_pd{{ $pd->id }}" class="form-label fw-bold">Deskripsi Pelanggaran <span
                                            class="text-danger">*</span></label>
                                    <textarea name="nama_pd" id="edit_nama_pd{{ $pd->id }}" class="form-control form-control-modern"
                                        rows="4" required>{{ $pd->nama_pd }}</textarea>
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-light btn-modern px-4" data-bs-dismiss="modal">Batal</button>
