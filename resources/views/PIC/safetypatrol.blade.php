...
<style>
    ...
    .table td { border: none !important; padding: 18px 20px !important; }
    ...
    .btn-after { background: linear-gradient(135deg, #f59e0b, #d97706); color: white; }
...
<div class="content p-4 animate-in">
    <div class="row align-items-center mb-5">
        <div class="col-md-7">
            <h6 class="text-primary fw-bold text-uppercase mb-2" style="letter-spacing: 2px;">PIC Control Panel</h6>
            <h1 class="display-5 fw-extrabold text-dark mb-2">Safety Patrol <span class="text-primary">Hub</span></h1>
        ...
        <div class="col-md-5 text-md-end">
            <a href="{{ url('/pic/safety-patrol/export') }}" class="btn btn-futuristic btn-export">
                <i class="bi bi-box-arrow-up-right"></i> Export CSV Data
            </a>
        </div>
    </div>
                ...
                <div class="d-flex flex-wrap gap-3 align-items-center">
                    ...
                    <div style="min-width: 180px;">
                        <select name="area" class="form-select filter-input" onchange="this.form.submit()">
                            ...
                            <option value="Produksi" {{ request('area') == 'Produksi' ? 'selected' : '' }}>Produksi</option>
                ...
                <div class="table-responsive">
                    <table class="table align-middle">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex flex-column">
                                        <span class="text-dark fw-bold">{{ \Carbon\Carbon::parse($laporan->tanggal)->format('d/m/y') }}</span>
                                        <small class="text-muted">{{ \Carbon\Carbon::parse($laporan->tanggal)->format('H:i') }}</small>
                                    </div>
                                </td>
                                <td>
                                    <div class="d-flex flex-column">
                                        <span class="badge bg-dark mb-1" style="width: fit-content;">{{ $laporan->eporte }}</span>
                                        <small class="text-primary fw-bold">{{ $laporan->section->section ?? 'N/A' }}</small>
                                    </div>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="text-truncate" style="max-width: 150px; cursor: help;" title="{{ $laporan->problem }}">
                                        {{ $laporan->problem }}
                                    </div>
                                </td>
                                <td class="text-center">
                                    <span class="fw-bold {{ \Carbon\Carbon::parse($laporan->due_date)->isPast() && $laporan->status != 'Close' ? 'text-danger' : 'text-muted' }}">
                                        {{ $laporan->due_date ? \Carbon\Carbon::parse($laporan->due_date)->format('d M') : '-' }}
                                    </span>
                                </td>
                                <td class="text-center">
                                    <div class="d-flex justify-content-center gap-2">
                                        <span class="badge {{ $laporan->foto_before ? 'bg-info' : 'bg-light text-muted' }}" title="Before">B</span>
                                        <span class="badge {{ $laporan->foto_after ? 'bg-success' : 'bg-light text-muted' }}" title="After">A</span>
                                    </div>
                                </td>
                                <td class="text-center">
                                    <span class="status-pill bg-{{ $statusBadge }} bg-opacity-10 text-{{ $statusBadge }}">
                                        ● {{ $laporan->status }}
                                    </span>
                                </td>
                                <td class="text-center">
                                    <div class="d-flex justify-content-center gap-2">
                                        ...
                                        <button type="button" class="btn btn-sm btn-outline-dark shadow-sm" style="border-radius: 10px;"
                            ...
                            @empty
                            <tr>
                                <td colspan="9" class="text-center py-5 text-muted">Data tidak ditemukan dengan filter tersebut.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div class="modal-body p-4 pt-0">
                    <form action="{{ url('/pic/safety-patrol/upload-after/' . $laporan->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row g-4">
                            <div class="col-md-5">
                                <div class="p-4 bg-light rounded-4 border-dashed border-2">
                                    ...
                                    <div class="mb-2"><small class="text-muted d-block">Masalah:</small> <strong>{{ $laporan->problem }}</strong></div>
                                    <div class="mb-3"><small class="text-muted d-block">Target:</small> <strong>{{ $laporan->due_date }}</strong></div>
                                </div>
                            </div>
                            <div class="col-md-7">
                        ...
                        <div class="text-end mt-4">
                            <button type="submit" class="btn btn-primary px-5 py-2 fw-bold shadow-lg" style="border-radius: 12px;">Kirim Solusi</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content overflow-hidden">
                ...
                <div class="modal-body p-4 pt-0">
                    <div class="row g-3">
                        <div class="col-md-6">
                            <div class="text-center mb-2"><span class="badge bg-danger bg-opacity-10 text-danger rounded-pill px-3">BEFORE</span></div>
                            <div class="rounded-4 overflow-hidden border" style="height: 300px;">
                                ...
                                @else
                                <div class="h-100 d-flex align-items-center justify-content-center bg-light text-muted">No Image</div>
                                @endif
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="text-center mb-2"><span class="badge bg-success bg-opacity-10 text-success rounded-pill px-3">AFTER</span></div>
                            <div class="rounded-4 overflow-hidden border shadow-sm" style="height: 300px;">
                                ...
                                @else
                                <div class="h-100 d-flex align-items-center justify-content-center bg-light text-muted italic">Waiting for update...</div>
                                @endif
                            </div>
                        </div>
                    </div>
