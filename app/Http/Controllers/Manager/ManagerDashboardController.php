...
class ManagerDashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // ambil semua section dalam department user (STRING)
        $sections = Section::where('department', $user->department)
            ->orderBy('section')
            ->get();
    
        // section terpilih (default = section user)
        $selectedSectionId = $request->input('section', $user->section_id);
    
        // 🔒 VALIDASI: pastikan section masih dalam department user
        if (! $sections->pluck('id')->contains($selectedSectionId)) {
            $selectedSectionId = $user->section_id;
        }
    
        $dashboardData = $this->getDashboardDataWithFilters(
            $request,
            $selectedSectionId
        );
     ...
     */
    private function getDashboardDataWithFilters(Request $request, $sectionId)
    ...
    public function getDashboardData(Request $request)
    {
        $startDate = $request->input('start_date', Carbon::now()->subDays(30)->format('Y-m-d'));
        $endDate = $request->input('end_date', Carbon::now()->format('Y-m-d'));
        $sectionId = $request->input('section', auth()->user()->section_id);
        $status = $request->input('status');
    ...
    public function exportDashboardData(Request $request)
{
    ...
    // 🔐 ambil section dari dropdown / fallback ke user
    $sectionId = $request->input('section', $user->section_id);

    // 🔐 VALIDASI: section HARUS masih satu department
    $allowedSections = Section::where('department', $user->department)
        ->pluck('id');
    ...
    // 🔁 PAKSA inject ke request supaya konsisten
    $request->merge([
        'start_date' => $startDate,
        'end_date'   => $endDate,
        'section'    => $sectionId,
    ...
    $data = $this->getDashboardDataWithFilters($request, $sectionId);

    // 🔽 nanti kalau pakai Excel
